...
fn test_buffer_creation_error_with_context() {
    let err =
        Error::BufferCreation("failed to create temporary file: Permission denied".to_string());
...
fn test_invert_failed_error() {
    ...
    assert!(err
        .to_string()
        .contains("Failed to apply Y-invert transformation"));
    assert!(err.to_string().contains("vertical flip"));
}

#[test]
fn test_scaling_failed_error() {
    let err = Error::ScalingFailed(
        "failed to create image buffer for scaling 1920x1080 -> 960x540".to_string(),
    );
